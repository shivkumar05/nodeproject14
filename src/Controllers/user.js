    ...
    try {
        ...
        if (req.body.time) {
            if (lastObject.time < req.body.time) {
                return res.status(400).json({ status: false, message: 'Cannot set routine for next 30 minutes' });
            }
        }
        // await routineModel.create(data);
        lastRoutineTime.savedData();
        return res.status(201).json({ message: "Routine set successfully", data: lastRoutineTime })
...
const getMyDrills = async function (req, res) {
    try {
        ...
        const drills = await myDrillModel.find({ userId: userid }).select({ createdAt: 0, updatedAt: 0, __v: 0 });
